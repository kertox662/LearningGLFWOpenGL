...
int main(void)
{
    ...
    glGenBuffers(1,&IBO);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO); //Make sure to use GL_ELEMENT_ARRAY_BUFFER
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_DYNAMIC_DRAW); //Sets the data in IBO
    ...
    while (!glfwWindowShouldClose(window)) //Render Loop while not closing window
    {
        ...
        //DrawElements for GL_ELEMENT_ARRAY_BUFFER
